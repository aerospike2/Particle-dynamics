...
class MinimalCellBox:
    ...
    def __init__(self,
        ...
        self.pType = np.concatenate([np.ones(self.init_state.shape[0]*ratio), np.ones(self.init_state.shape[0]*(1-ratio))])       
    ...
    def step(self, dt):
        ...
        for i1, i2 in zip(ind1, ind2):
            ...
            if self.pType[i1] == 1 and self.pType[i2] == 1:
               self.state[i1, 2:] = (v_cm + v_rel * m2 / (m1 + m2))
               self.state[i2, 2:] = (v_cm - v_rel * m1 / (m1 + m2)) 
            
            elif self.pType[i1] == 2 and self.pType[i2] == 2:             
               self.state[i1, 2:] = (v_cm + v_rel * m2 / (m1 + m2))
               self.state[i2, 2:] = (v_cm - v_rel * m1 / (m1 + m2))                                             
...
init_pos = 3.9*(-0.5 + np.random.random((100, 2)))
init_vel = 0.05*(-0.5 + np.random.random((100, 2)))
...
cells, = ax.plot([], [], color = 'c', marker = 'o', ls = '', ms=6)
...
def animate(i):
    ...
    cells.set_data(box.state[:, 0], box.state[:, 1])
    cells.set_markersize(ms)
    return cells, rect
